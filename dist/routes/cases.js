...
router.get('/:id', auth_1.authenticate, async (req, res) => {
    try {
        const caseDoc = await Case_1.default.findById(req.params.id)
        ...
        const isParty = caseDoc.citizenId?._id?.toString() === userId ||
...
router.patch('/:id/status', auth_1.authenticate, async (req, res) => {
    ...
    try {
        const caseDoc = await Case_1.default.findByIdAndUpdate(req.params.id, { status, lastActivityAt: new Date() }, { new: true });
        if (!caseDoc) {
            res.status(404).json({ error: 'Case not found' });
            return;
        }
        ...
        await (0, firebaseNotify_1.firebaseNotify)({
            ...
            userIds: [caseDoc.citizenId, caseDoc.lawyerId, caseDoc.judgeId].filter(Boolean),
        });
        res.json({ case: caseDoc });
    }
    catch (error) {
...
router.post('/:id/grievance', auth_1.authenticate, async (req, res) => {
    ...
    try {
        const caseDoc = await Case_1.default.findByIdAndUpdate(req.params.id, {
            $push: {
                grievances: {
                    raisedBy: user._id,
                    message,
...
router.patch('/:id/assign', auth_1.authenticate, async (req, res) => {
    ...
    try {
        ...
        if (judgeId)
            updates.judgeId = judgeId;
        if (lawyerId)
            updates.lawyerId = lawyerId;
        const caseDoc = await Case_1.default.findByIdAndUpdate(req.params.id, updates, { new: true });
        if (!caseDoc) {
            res.status(404).json({ error: 'Case not found' });
            return;
        }
