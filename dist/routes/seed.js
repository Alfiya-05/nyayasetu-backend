...
router.post('/lawyers', async (_req, res) => {
    ...
    try {
        ...
        for (const data of DEMO_LAWYERS) {
            ...
            const fakeUid = `demo_${new mongoose_1.default.Types.ObjectId().toHexString()}`;
...
router.delete('/lawyers', async (_req, res) => {
    if (process.env.NODE_ENV === 'production') {
        res.status(403).json({ error: 'Not allowed in production' });
        return;
    }
    const demoEmails = DEMO_LAWYERS.map(l => l.email);
    const users = await User_1.default.find({ email: { $in: demoEmails } });
    const userIds = users.map(u => u._id);
    await LawyerProfile_1.default.deleteMany({ userId: { $in: userIds } });
    await User_1.default.deleteMany({ email: { $in: demoEmails } });
